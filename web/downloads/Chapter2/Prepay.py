...
# 1) Computes daily-compounded amortization with optional extra monthly payments and early-payment offset.
# 2) Produces two publication-quality figures (line + bar), with clean typography and finance-friendly axis formatting.
# 3) Saves a CSV summary of scenarios for inclusion in your book/paper.
#
# Notes:
# - Uses matplotlib only (no seaborn), one chart per figure.
# - Fonts fall back gracefully if Times New Roman is not available.
# - Figures are saved under ./figures/Chapter2/
...
early_days = 29  # scenario: pay 14 days before the month end
...
eff_mr = (1 + daily_rate) ** days_per_month - 1  # effective monthly rate
...
def daily_amortization_schedule(P, daily_r, months, monthly_pmt, *, extra_payment=0.0, early_days=0, dpm=30):
    """Simulate a daily-compounded loan with monthly payments."""
